...
class NftablesTestCase(unittest.TestCase):
    ...
    def assert_commands_equal(self, actual_commands: List[str], expected_commands: List[str]):
        # Normalize commands by removing potential nft executable path if present
        # and stripping extra whitespace from each command string.
        # The command from fwsimple is already a string generated by list2cmdline.
        normalize = lambda cmd_str: cmd_str.replace("nft ", "", 1).strip()
        ...
        for i, actual_cmd in enumerate(actual_normalized):
            ...
            # or have different spacing. e.g. { type filter hook input priority 0 ; policy accept ; }
...
"""
        ...
        expected_commands = [
            ...
            "nft add rule inet fwsimple input ip version 4 icmp type 3 code 4 accept comment \"\\\"[ICMP] Fragmentation needed\\\"\"",
            "nft add rule inet fwsimple input ip version 4 icmp type 3 code 3 accept comment \"\\\"[ICMP] Port unreachable\\\"\"",
            "nft add rule inet fwsimple input ip version 4 icmp type 3 code 1 accept comment \"\\\"[ICMP] Host unreachable\\\"\"",
